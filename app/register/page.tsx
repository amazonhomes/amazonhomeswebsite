...
export default function RegisterPage() { 
  ...
  async function handleSubmit(e: React.FormEvent) { 
    e.preventDefault() 
    const phoneCheck = validatePhoneField(form.phone, { required: true })
    if (!phoneCheck.ok) {
      setPhoneError(phoneCheck.error)
      return
    }
    setSubmitting(true) 
    // Submit the canonical E.164 value so the account is created with a
    // normalized phone in its metadata/profile.
    const result = await register({ ...form, phone: phoneCheck.value })
    setSubmitting(false) 
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4"> 
            ...
            <div className="flex flex-col gap-1.5"> 
              ...
              <PhoneInput
                ...
                required 
                invalid={!!phoneError}
                describedBy={phoneError ? 'r-phone-error' : undefined}
              /> 
              {phoneError && (
                <p id="r-phone-error" className="text-sm text-destructive">
                  {phoneError}
                </p>
              )}
            </div> 
