                ...
                <div className="group relative">
                  ...
                  {property.photos.length > 1 && (
                    <button
                      ...
                      className="absolute left-2 top-1/2 z-20 flex size-9 -translate-y-1/2 items-center justify-center rounded-full border border-border bg-background/90 shadow-md backdrop-blur-sm transition hover:bg-background"
                  ...
                  <div
                    ref={carouselRef}
                    className="flex snap-x snap-mandatory gap-3 overflow-x-auto px-1 py-1 scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                  >
                    {property.photos.map((photo, i) => (
                      <button
                        key={`${photo.url || photo.previewUrl || "photo"}-${i}`}
                        type="button"
                        onClick={() => setActivePhoto(i)}
                        className={cn(
                          "relative aspect-square shrink-0 snap-start overflow-hidden rounded-md ring-2 transition",
                          "w-[calc((100%-0.75rem)/2)] sm:w-[calc((100%-1.5rem)/3)] md:w-[calc((100%-2.25rem)/4)]",
                          i === activePhoto
                            ? "ring-accent"
                            : "ring-transparent",
                        )}
                  ...
                  {property.photos.length > 1 && (
                    <button
                      ...
                      className="absolute right-2 top-1/2 z-20 flex size-9 -translate-y-1/2 items-center justify-center rounded-full border border-border bg-background/90 shadow-md backdrop-blur-sm transition hover:bg-background"
              ...
              {isAuthed ? (
                ...
                <div className="mt-3 flex items-center justify-between gap-4 rounded-lg border border-dashed border-border p-4">
                  <p className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    The exact location and map unlock
                    after you register.
                  </p>
            ...
            <div>
              ...
              {isAuthed ? (
                ...
                <div className="mt-2 flex items-center justify-between gap-4 rounded-md border border-dashed border-border p-4">
                  <p className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Lock className="size-4" />
                    Showing details are visible to
                ...
                offersClosed ? (
                  <div className="flex flex-col gap-2.5">
                    <p className="rounded-md bg-secondary px-3 py-2 text-center text-sm text-muted-foreground">
                      Offer submission for this property
                      has closed.
                    </p>
